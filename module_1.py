...
while True:
    ...
    if run.lower() == 'n':
        ...
        # site density
        C_l = int(input('Enter coating concentration: '))
        coating_concs.append(C_l)
        
        # flow rate
        Q = input('For coating concentration = %d, enter flow rates (microliter/hour), separated by commas and without new lines: ' % C_l)
        ...
        for i in range(len(Q_arr)):
            track_file_name = input('For flow rate = %.2f and coating concentration = %d, enter name of "Track statistics" file (s) from Trackmate: ' % (Q_arr_nc[i], C_l))
            ...
            spots_file_name = input('For flow rate = %.2f and coating concentration = %d, enter name of "Spots in track statistics" file (s) from Trackmate: ' % (Q_arr_nc[i], C_l))
            ...
            t_min_input = input('If applicable, for flow rate = %.2f and coating concentration = %d, enter non-specific binding time(s). Otherwise, enter \"n\" for all trials: ' % (Q_arr_nc[i], C_l))
...
for m in range(len(track_data)):
    ...
    for n in range(len(track_data[m])):
        ...
        u_f = y*shear_rates[m][n]*(1-(5/16)*(a/y)**3) * 1e-6 # convert back to microns/s
